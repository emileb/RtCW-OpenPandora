...
void Sys_Error( const char *error, ... ) {
	...
	va_end( argptr );
	 ...
	 ((void)__android_log_print(ANDROID_LOG_ERROR,"JK2", string));
...
void Sys_Warn( char *warning, ... ) {
	...
	((void)__android_log_print(ANDROID_LOG_INFO,"RTCW", string));
...
void Sys_UnloadDll( void *dllHandle ) {
	...
	Com_Printf( "Sys_UnloadDll %p",dllHandle);
...
void  Sys_Print( const char *msg ) {
	...
	((void)__android_log_print(ANDROID_LOG_INFO,"RTCW", msg));
